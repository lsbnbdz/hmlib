...
- **歌词组件性能优化（V1/V2 同步）**
  - 修复歌词初始化时因瞬间计算量过大导致的卡顿问题
  - 位置计算由 O(n²) 改为前缀和 O(n)：100 行歌词初始化计算量从约 50 万次迭代降至约 200 次
  - 行高度变化批处理：初始化时 n 次 `onAreaChange` 只触发一次位置重算（去抖合并），高度数组改为元素赋值，不再整组拷贝
  - 逐字进度（`currentLineElapsed`）仅传播给播放行：播放期间子组件刷新量从每秒 125×n 次降至每秒 125 次
  - 链式动画单行 setter 改为元素赋值：去掉每次 O(n) 的 `.map()` 数组拷贝
  - V1 默认值常量提升（`DEFAULT_SPRING_CONST` / `DEFAULT_CONFIG_CONST`）：热路径零对象分配

- **新增公共计算工具类**
  - 新增 `utils/LyricsCalcUtils.ets`，收拢 V1/V2 重复的纯计算逻辑（key 生成、位置计算、限位、二分查找），后续算法只维护一处
  - 工具类为内部实现，未导出到公共 API

### 兼容性

- 对外 API 零变更：组件属性、类型定义、导出清单、回调签名均未修改
- 存量调用方无需任何改动即可升级
